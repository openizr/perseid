/**
 * Copyright (c) Selfcity Inc.
 * All rights reserved.
 */

import { REPLServer } from 'repl';
import { Id } from '@perseid/core';
import {
  Model,
  Logger,
  EmailClient,
  CacheClient,
  OAuthEngine,
  DatabaseClient,
} from '@perseid/server';

/**
 * Node REPL, with all services initialized.
 * This prompt allows you to test services in real conditions.
 */

const repl = new (REPLServer as any)();
(repl.setupHistory as REPLServer['setupHistory'])('/var/www/html/server/node_modules/.node_repl', async (_e, newRepl) => {
  const Binary = (value: string): ArrayBuffer => {
    const textEncoder = new TextEncoder();
    return textEncoder.encode(value).buffer;
  };

  const logger = new Logger({ logLevel: 'debug', prettyPrint: true });


  const model = new Model({
    types: {},
    collections: {},
  });

  const emailClient = new EmailClient();

  const cacheClient = new CacheClient();

  const databaseClient = new DatabaseClient(model, logger, cacheClient, {
    host: 'mongodb',
    port: 27017,
    user: null,
    password: null,
    protocol: 'mongodb:',
    database: 'test',
    connectTimeout: 2000,
    cacheDuration: 0,
    maxPoolSize: 2,
    connectionLimit: 10,
    queueLimit: 0,
  });

  const engine = new OAuthEngine(
    model,
    logger,
    emailClient,
    cacheClient,
    databaseClient,
    {
      baseUrl: 'https://test.test',
      oAuth: {
        algorithm: 'RS256',
        clientId: 'example',
        issuer: 'example',
        privateKey: '-----BEGIN RSA PRIVATE KEY-----\\n6iX5ubz53VpYgjKf2ebZjjnBR+OeufD377OzY7/uRLdaJf22p1RW\n-----END RSA PRIVATE KEY-----',
        publicKey: '-----BEGIN RSA PUBLIC KEY-----\\n-----END RSA PUBLIC KEY-----',
      },
    }
  );

  Object.assign(newRepl.context, {
    Id,
    Date,
    model,
    logger,
    Binary,
    engine,
    emailClient,
    cacheClient,
    databaseClient,
    context: { user: (await databaseClient.list('users', { fields: ['_id'] })).results[0] },
  });

  newRepl.displayPrompt();
});
