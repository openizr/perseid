...
import FastifyController from '@perseid/server/fastify';
import fastify, { type FastifyBaseLogger } from 'fastify';

// Works exactly the same with the `ExpressController` service.
...
async function main() {
  ...
  app.listen({ port: parseInt(String(process.env.BACKEND_PORT), 10), host: '0.0.0.0' }, (error) => {
    if (error) {
      logger.fatal(error);
      process.exit(1);
    }
