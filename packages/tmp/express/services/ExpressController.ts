...
export interface ExpressCustomEndpoint<
  DataModel extends UsersDataModel
...
> extends Controller<DataModel, Model, Engine> {
  ...
  protected apiHandlers: Record<string, ExpressCustomEndpoint<DataModel>> = {
    _model: {
      ...
      query: {
        fields: {
          resource: { type: 'string', isRequired: true, maxLength: 100 },
        },
      },
    },
    signUp: {
    ...
    signIn: {
      ...
      body: {
        fields: {
          email: { type: 'string', isRequired: true, maxLength: 320 },
          password: { type: 'string', isRequired: true, maxLength: 500 },
        },
      },
    },
    viewMe: {
        ...
        if (endpoint === 'create') {
          instance.post(`${prefix}${path}`, this.createEndpoint({
            ...
            handler: async (request, response) => {
              ...
              const fullOptions = {
                ...request.query as ViewCommandOptionsWithoutKey,
                maximumDepth,
              };
          ...
          instance.get(`${prefix}${path}`, this.createEndpoint({
            ...
            handler: async (request, response) => {
              const params = request.params as unknown as CommandContext<DataModel>;
              const fullOptions = {
                ...request.query as ListCommandOptionsWithoutKey,
                maximumDepth,
              };
              const results = await this.engine.list(resource, {
                query: null,
                filters: null,
              }, fullOptions, params);
          ...
          instance.post(`${prefix}${path}`, this.createEndpoint({
            ...
            handler: async (request, response) => {
              ...
              const results = await this.engine.list(resource, searchBody, fullOptions, params);
