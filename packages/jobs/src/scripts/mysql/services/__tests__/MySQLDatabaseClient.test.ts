...
type TestDatabase = MySQLDatabaseClient & {
  client: MySQLDatabaseClient['client'];
  database: MySQLDatabaseClient['database'];
  formatTasks: MySQLDatabaseClient['formatTasks'];
};
...
describe('mysql/services/MySQLDatabaseClient', () => {
  ...
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    databaseClient = new MySQLDatabaseClient(logger, cache, {
      protocol: '',
      host: '',
      port: null,
      user: null,
  ...
  describe('[updateMatchingTask]', () => {
    test('database error', async () => {
      ...
      expect((await databaseClient.client.getConnection()).release).toHaveBeenCalledOnce();
    });
    ...
    test('no error', async () => {
      ...
      const query = 'UPDATE `tasks` SET\n  `_status` = ?,\n  `_runBy` = ?\nWHERE\n  `_runBy` IS'
        + ' NULL\n  AND `_status` = ?\n  AND `_id` = ?;';
      const log1 = '[MySQLDatabaseClient][updateMatchingTask] Performing the following SQL query on database:';
      const log2 = `[MySQLDatabaseClient][updateMatchingTask]\n\n${query}\n`;
      const log3 = '[MySQLDatabaseClient][updateMatchingTask] [\n  CANCELED,\n  000000000000000000000001,\n  PENDING,\n  000000000000000000000010\n]\n';
      expect(databaseClient.client.getConnection).toHaveBeenCalledOnce();
      expect((await databaseClient.client.getConnection()).execute).toHaveBeenCalledOnce();
      expect((await databaseClient.client.getConnection()).execute).toHaveBeenCalledWith(query, [
        'CANCELED',
        '000000000000000000000001',
        'PENDING',
        '000000000000000000000010',
  ...
  test('[getRunningTasks]', async () => {
    ...
    const query = 'SELECT\n  `tasks`.`_id` AS `_id`,\n  `tasks`.`_createdAt` AS '
      ...
      + '`job`.`_id`\nWHERE\n  `tasks`.`_status` = ?;';
    const log1 = '[MySQLDatabaseClient][getRunningTasks] Performing the following SQL query on database:';
    const log2 = `[MySQLDatabaseClient][getRunningTasks]\n\n${query}\n`;
    const log3 = '[MySQLDatabaseClient][getRunningTasks] [\n  IN_PROGRESS\n]\n';
    expect(logger.debug).toHaveBeenCalledTimes(3);
    expect(logger.debug).toHaveBeenCalledWith(log1);
    expect(logger.debug).toHaveBeenCalledWith(log2);
    expect(logger.debug).toHaveBeenCalledWith(log3);
  ...
  test('[getCandidatePendingTasks]', async () => {
    ...
    const query = 'SELECT\n  `tasks`.`_id` AS `_id`,\n  `tasks`.`_createdAt` AS `_createdAt`,\n  '
      ...
      + '`startAt` <= ? OR `startAfter`.`_status` = ?);';
    const log1 = '[MySQLDatabaseClient][getCandidatePendingTasks] Performing the following SQL query on database:';
    const log2 = `[MySQLDatabaseClient][getCandidatePendingTasks]\n\n${query}\n`;
    const log3 = '[MySQLDatabaseClient][getCandidatePendingTasks] [\n  PENDING,\n  Sat Jan 01 2022 00:00:00 GMT+0000 (Coordinated Universal Time),\n  COMPLETED\n]\n';
    expect(logger.debug).toHaveBeenCalledTimes(3);
    expect(logger.debug).toHaveBeenCalledWith(log1);
    expect(logger.debug).toHaveBeenCalledWith(log2);
    expect(logger.debug).toHaveBeenCalledWith(log3);
