    ...
    return this.handleError(async () => {
      ...
      filterFields.forEach((fieldName) => {
        ...
        if (value === null) {
          ...
          sqlFilters.push(`\`${fieldName}\` = ?`);
          values.push(value instanceof Id ? String(value) : value);
        }
      });
      const placeholders = fieldPlaceholders.join(',\n  ');
      const sqlQuery = `UPDATE \`tasks\` SET\n  ${placeholders}\nWHERE\n  ${sqlFilters.join('\n  AND ')};`;
      this.logger.debug('[MySQLDatabaseClient][updateMatchingTask] Performing the following SQL query on database:');
      this.logger.debug(`[MySQLDatabaseClient][updateMatchingTask]\n\n${sqlQuery}\n`);
      this.logger.debug(`[MySQLDatabaseClient][updateMatchingTask] [\n  ${values.join(',\n  ')}\n]\n`);
      const connection = await this.client.getConnection();
      try {
        const [response] = await connection.execute(sqlQuery, values);
        connection.release();
        return (response as { affectedRows: number; }).affectedRows === 1;
      } catch (error) {
        connection.release();
        throw error;
      }
  ...
  public async getRunningTasks(): Promise<DataModel['tasks'][]> {
    return this.handleError(async () => {
      ...
      const fullSQLQuery = 'SELECT\n  `tasks`.`_id` AS `_id`,\n  `tasks`.`_createdAt` AS '
        ...
        + '`job`.`_id`\nWHERE\n  `tasks`.`_status` = ?;';
      this.logger.debug('[MySQLDatabaseClient][getRunningTasks] Performing the following SQL query on database:');
      this.logger.debug(`[MySQLDatabaseClient][getRunningTasks]\n\n${fullSQLQuery}\n`);
      this.logger.debug(`[MySQLDatabaseClient][getRunningTasks] [\n  ${values.join(',\n  ')}\n]\n`);
      const [response] = await this.client.query(fullSQLQuery, values) as mysql.RowDataPacket[][];
      return this.formatTasks(response);
    });
  }
  ...
  public async getCandidatePendingTasks(): Promise<DataModel['tasks'][]> {
    return this.handleError(async () => {
      ...
      const fullSQLQuery = 'SELECT\n  `tasks`.`_id` AS `_id`,\n  `tasks`.`_createdAt` AS '
        ...
        + '\n  `tasks`.`_status` = ?\n'
        + '  AND (`tasks`.`startAt` <= ? OR `startAfter`.`_status` = ?);';
      this.logger.debug('[MySQLDatabaseClient][getCandidatePendingTasks] Performing the following SQL query on database:');
      this.logger.debug(`[MySQLDatabaseClient][getCandidatePendingTasks]\n\n${fullSQLQuery}\n`);
      this.logger.debug(`[MySQLDatabaseClient][getCandidatePendingTasks] [\n  ${values.join(',\n  ')}\n]\n`);
      const [response] = await this.client.query(fullSQLQuery, values) as mysql.RowDataPacket[][];
      return this.formatTasks(response);
    });
